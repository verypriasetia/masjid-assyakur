...
// UX Optimal: Jeda Iqamah disesuaikan per waktu sholat (dalam menit)
...
async function muatDataGoogleSheets() {
    try {
        ...
        console.error("Gagal sinkronisasi data Google Sheets, beralih menggunakan localStorage cache:", error);
        tampilkanDataDariCacheLokal();
    }
}
...
function tampilkanDataDariCacheLokal() {
    ...
    if (cacheData) {
        const valueRanges = JSON.parse(cacheData);
        prosesDanTampilkanData(valueRanges);
    } else {
        ...
        document.getElementById('kas-awal').innerText = 'Rp 0';
        document.getElementById('kas-masuk').innerText = 'Rp 0';
        document.getElementById('kas-keluar').innerText = 'Rp 0';
        document.getElementById('kas-saldo').innerText = 'Rp 0';
    }
}
...
function prosesDanTampilkanData(valueRanges) {
    ...
    for (let i = 1; i < dataKeuangan.length; i++) {
        ...
        if (keterangan.includes("SALDO AWAL")) {
            // Card tetap tampil normal meskipun awal bertuliskan Rp 0
        ...
        if (saldoStr !== "" && saldoStr !== "0") {
            saldoAkhir = formatMataUangAman(saldoStr, false);
        }
    }
    ...
    for (let i = 1; i < dataKeuangan.length; i++) {
        ...
        // Sembunyikan Rp0 khusus untuk tabel di papan informasi tengah (diubah menjadi "-")
        let masukTabel = formatMataUangAman(baris[2], true);
        let keluarTabel = formatMataUangAman(baris[3], true);
        let saldoTabel = formatMataUangAman(baris[4], true);

        tableRowsHtml += `
            <tr>
                <td class="text-center">${baris[0] || '-'}</td>
                <td>${baris[1] || '-'}</td>
                <td class="text-right">${masukTabel}</td>
                <td class="text-right">${keluarTabel}</td>
                <td class="text-right" style="font-weight:600; color:#e5c158;">${saldoTabel}</td>
            </tr>
        `;
    }
...
// Fitur Sembunyikan Rp0: Parameter sembunyikanJikaNol ditambahkan untuk fleksibilitas tabel
function formatMataUangAman(teks, sembunyikanJikaNol = false) {
    if (!teks || teks === "0" || teks === "-" || teks.toString().trim() === "") {
        return sembunyikanJikaNol ? "-" : "Rp 0";
    }
    ...
    if (elemenScroll.scrollHeight > elemenScroll.clientHeight) {
        ...
        setTimeout(() => {
            scrollInterval = setInterval(() => {
                if (isModeSholatBerlangsung) { clearInterval(scrollInterval); return; }
