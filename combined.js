...
// DAFTAR GAMBAR OTOMATIS DIMUAT DARI GOOGLE SHEETS
...
setInterval(() => {
    ...
    const menit = Clinical = sekarang.getMinutes();
    const detik = sekarang.getSeconds();
    const sekarangDetik = (jam * 3600) + (menit * 60) + detik;
    
    // Update Jam Real-Time di Kolom Kanan
    ...
    // FIX TAMPILAN JADWAL SHOLAT BERIKUTNYA DI KOLOM KANAN
    ...
    // REVISI: Sintaksis perhitungan mundur dibersihkan total dari typo eror
    ...
    // Trigger Alarm Adzan (7 detik sebelum masuk waktu)
    ...
    // LOGIKA PAPAN KIRI UTAMA (INTERUPSI COUNTDOWN IQAMAH & STANDBY SHOLAT)
    ...
    if (iqamahAktif) {
        ...
        // Trigger BEEP Iqamah 7 detik sebelum habis
...
function aktifkanModeStandbySholat() {
    ...
    const htmlStandbyGambar = `<img src="waktu_sholat.jpg" class="slide-stretched-img" style="width:100%; height:100%; object-fit:fill; display:block;" onerror="this.onerror=null; this.src='logo.png';">`;
...
function bangunStrukturSlideAntrian() {
    ...
    // Ekstraksi daftar nama file gambar murni dari sheet INFOUPDATE LAINNYA
    DAFTAR_GAMBAR_LOKAL = [];
    ...
    // Ekstraksi daftar teks murni dari sheet INFOUPDATE LAINNYA
    const daftarTeksMurni = dataInfoLain.map(row => row[0]).filter(teks => teks && !teks.trim().match(/\.(jpg|jpeg|png)$/i));

    dataSlides = [];

    // ==========================================================================
    // SEKSI 1: REVISI SIKLUS GAMBAR & TEKS BERGANTIAN SEBANYAK 2 KALI DI AWAL
    // ==========================================================================
    
    // Perputaran ke-1 (Gambar 1 -> Teks 1)
    tambahkanItemGambarDinamis();
    tambahkanItemTeksDinamis(daftarTeksMurni);

    // Perputaran ke-2 (Gambar 2 -> Teks 2)
    tambahkanItemGambarDinamis();
    tambahkanItemTeksDinamis(daftarTeksMurni);

    // ==========================================================================
    // SEKSI 2: PENGUMUMAN JUMAT & KAS KEUANGAN MASJID
    // ==========================================================================
            ...
            <div class="padded-slide-inner" style="justify-content: space-between; padding: 2vh 2vw; height: 100%;">
                ...
                <div style="display: flex; gap: 1.5vw; width: 100%;">
                    <div style="flex: 1; background: rgba(46, 204, 113, 0.1); border: 0.18vh solid rgba(46, 204, 113, 0.4); border-radius: 1vh; padding: 1.5vh; text-align: center;">
                        ...
                        <strong style="font-size: 3.5vh; color: #ffffff; font-weight: 700; margin-top: 0.5vh; display: block;">${"Rp " + totalPemasukan.toLocaleString('id-ID')}</strong>
                    </div>
                    <div style="flex: 1; background: rgba(231, 76, 60, 0.1); border: 0.18vh solid rgba(231, 76, 60, 0.4); border-radius: 1vh; padding: 1.5vh; text-align: center;">
                        ...
                        <strong style="font-size: 3.5vh; color: #ffffff; font-weight: 700; margin-top: 0.5vh; display: block;">${"Rp " + totalPengeluaran.toLocaleString('id-ID')}</strong>
                    </div>
                </div>
